...
public class Settings {
    ...
    @ApiOperation(value = "Get specific setting category",
                  notes = "Get all settings that are under the specified category group. for example all downloader " +
                          "settings")
    ...
    public Response getSettingsGroup(
        ...
        if (setting.isEmpty())
            return Response.status(HttpServletResponse.SC_NOT_FOUND)
                           .entity(new ErrorMessage("Could not get group", "There is no group with that ID"))
                           .build();
        Setting s = (Setting) setting.get(0);
        if (s.type.toUpperCase().equals("GROUP"))
            return Response.status(HttpServletResponse.SC_BAD_REQUEST)
                           .entity(new ErrorMessage("Could not get group",
                                                    "The setting with the supplied ID was not a group type"))
                           .build();
    ...
    @Path("/structure/poll")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Long polls the structure endpoint",
                  notes = "Replies like the /structure endpoint if some settng is updated")
    @ApiResponses({@ApiResponse(code = HttpServletResponse.SC_OK,
    ...
    public Response updateSetting(
        ...
        if (dbRes.isEmpty())
            return Response.status(HttpServletResponse.SC_NOT_FOUND)
                           .entity(new ErrorMessage("Could not update setting", "No setting with that ID was found"))
                           .build();
        ...
        if (s.type.toUpperCase().equals("GROUP"))
            return Response.status(HttpServletResponse.SC_NOT_MODIFIED)
                           .entity(new ErrorMessage("Settings not updated",
                                                    "Setting is a group. Groups cannot change value"))
                           .build();
        ...
        if (!value.matches(regex))
            return Response.status(HttpServletResponse.SC_BAD_REQUEST)
                           .entity(new ErrorMessage("Did not update settings", "Input did not fit pattern"))
                           .build();
        s.value = value;
